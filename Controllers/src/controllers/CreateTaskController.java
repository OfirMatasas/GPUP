...
public class CreateTaskController implements Initializable {
    ...
    public class TaskThreadWatcher extends Thread
    {
        ...
        public void run()
        {
            ...
            CreateTaskController.this.PauseButton.setDisable(false);
            CreateTaskController.this.stopButton.setDisable(false);

            while(CreateTaskController.this.taskThread.isAlive())
            {
                if(CreateTaskController.this.taskThread.getStatusChanged())
                    taskPausedOrStopped();
            }

            disableTaskOptions(false);
            CreateTaskController.this.PauseButton.setDisable(true);
            CreateTaskController.this.stopButton.setDisable(true);

            Platform.runLater(() -> CreateTaskController.this.PauseButton.setText("Pause"));
        }

        public void taskPausedOrStopped()
        {
            if(CreateTaskController.this.taskThread.getStopped()) //Stopped
            {
                if(!CreateTaskController.this.taskThread.getPaused())
                    CreateTaskController.this.logTextArea.appendText("\nWaiting for the task to stop...\n\n");

                while(!CreateTaskController.this.taskThread.getExecutor().isTerminated()) {}

                Platform.runLater(() -> CreateTaskController.this.logTextArea.appendText("\nTask stopped!\n\n"));
            }
            else //Paused / Resumed
            {
                ...
                if(CreateTaskController.this.taskThread.getPaused()) //Paused
                {
                ...
                CreateTaskController.this.PauseButton.setDisable(true);
                CreateTaskController.this.stopButton.setDisable(true);
                CreateTaskController.this.logTextArea.appendText(firstOutput);

                if(CreateTaskController.this.taskThread.getPaused())
                    while(!CreateTaskController.this.taskThread.getExecutor().isTerminated()) {}

                CreateTaskController.this.updateThreadButton.setVisible(updateThread);
                CreateTaskController.this.updateThreadButton.setDisable(!updateThread);
                CreateTaskController.this.numberOfThreadToExecuteLabel.setDisable(!updateThread);
                CreateTaskController.this.threadsSpinner.setDisable(!updateThread);

                String finalSecondOutput = secondOutput;
                Platform.runLater(() ->
                        {
                            CreateTaskController.this.logTextArea.appendText(finalSecondOutput);
                            CreateTaskController.this.PauseButton.setText(newButtonText);
                        });
                CreateTaskController.this.PauseButton.setDisable(false);
                CreateTaskController.this.stopButton.setDisable(false);
            }

            CreateTaskController.this.taskThread.resetStatusChanged();
        }
    }
    ...
    private void createNewProgressBar()
    {
        Task<Void> task = new Task<Void>() {
            ...
            protected Void call() throws Exception {
                int maxSize = CreateTaskController.this.taskTargetDetailsTableView.getItems().size();
                while (CreateTaskController.this.taskThread.isAlive()) {
                    Thread.sleep(200);
                    getFinishedTargetsInRealTime();
                    updateProgress(CreateTaskController.this.finishedTargets, maxSize);
                }
        ...
        this.taskTargetDetailsTableView.getItems().addListener(new ListChangeListener<TaskTargetInformation>() {
            ...
            public void onChanged(Change<? extends TaskTargetInformation> c) {
               ...
               // TaskController.this.clearTableButton.setDisable(c.getList().isEmpty());
                ...
                CreateTaskController.this.incrementalRadioButton.setDisable(!incrementalIsOptional());
            }
        });
    }
